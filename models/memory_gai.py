...
from models1.gateCNN2_gai4 import GatedCNNBlock,KANLinear
...
class MemoryMatrixBlock(nn.Module):
    ...
    '''
    def __init__(self, config, in_channels,dim):
        ...
        if config.memory_layer_type == 'default':
            for i in range(config.num_memory):
                ...
                self.memory1.append(Memory(config.num_slots, in_channels, config.shrink_thres))
        elif config.memory_layer_type == 'dim_reduce':
            for i in range(config.num_memory):
                self.memory.append(DimReduceMemory(config, in_channels))

        self.gateCNN=GatedCNNBlock(dim,dim)
        self.fc3=KANLinear(dim*2,dim)

    def forward(self, x):
        B, C, H, W = x.size()
        memory_start = time.process_time()
        ox = x
        x,x1=self.gateCNN(x,x)
        ...
        x1 = basic_modules.make_window(x1, window_size, stride=window_size, padding=0)
        ...
        alphas1 = list()
        for i in range(self.num_memory):
            m1, alpha1 = self.memory1[i](x1[:, i, :])
            mem_styles1[:,i,:] = mem_styles1[:,i,:] + m1
        ...
        x1 = mem_styles.view(-1, C, window_size, window_size)
        x = basic_modules.window_reverse(x, window_size, H, W)
        x1 = basic_modules.window_reverse(x1, window_size, H, W)
