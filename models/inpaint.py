...
class InpaintBlock(nn.Module):
    def __init__(self, config, num_in_ch, num_memory=9):
        ...
        if config.use_memory_queue:
            for _ in range(num_memory):
                self.memory1.append(
                    memory.MemoryQueue(config.num_slots, memory_channel, shrink_thres=config.shrink_thres))
        else:
            for _ in range(num_memory):
                self.memory1.append(memory.Memory(config.num_slots, memory_channel, config.shrink_thres))
        ...
        self.fff=FFML(512)
        self.sss=SpatialAttention_max(512)
        self.fffc=FC(1024)
    def forward(self, x, bs, num_windows, add_condition=False):
        ...
        f_x1 = self.bottleneck_conv1(x1)
        f_x1 = f_x1.contiguous().view(bs, num_windows, -1)
        ...
        n_x1 = n_x1.view(bs, num_windows, OC // 4, H, W)
        # print(n_x.shape,"n_x.shape")
        f_x = n_x
        f_x1 = n_x1
        # print(add_condition,"add_condition")
        # add position condition, this may help?
        if add_condition:
            f_x = self.add_condition(f_x, bs, num_windows) # bs, num_windows, OC // 4, H, W
            f_x1 = self.add_condition(f_x1, bs, num_windows)

        f_x = f_x.view(bs*num_windows, -1, H, W)
        f_x1 = f_x1.view(bs * num_windows, -1, H, W)
        # print(f_x.shape,"f_x1111")

        # bottleneck
        f_x = self.bottleneck_conv2(f_x) # B, OC, H, W
        f_x1 = self.bottleneck_conv2(f_x1)
        # f_x =f_x+f_x1
        f_x2=torch.cat([f_x,f_x1],dim=1)
        f_x=self.fffc(f_x2)
        # print(f_x.shape, "f_x2222")
        if self.training:
            # spatial binary mask
