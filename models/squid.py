...
# from models.memory_gai import MemoryMatrixBlock, \
#                    MemoryMatrixBlockV2, \
#                    MemoryMatrixBlockV3, \
#                    MemoryMatrixBlockV4
# from models2.tr import *
from models2.tr_1 import *
from models.memory import MemoryMatrixBlock, \
                   MemoryMatrixBlockV2, \
                   MemoryMatrixBlockV3, \
                   MemoryMatrixBlockV4
from models.inpaint_yuan import InpaintBlock

# from models1.KANmamba1 import *
from models1.ConvSSMgai3 import *
from models1.CBAMgai1 import CBAM
class AE(nn.Module):
    def __init__(self, config, features_root, level=4):
        ...
        self.dim_list=[256,128]
        self.size=[4,8]
        self.in_channels=[256,128]
        ...
        # ])
        self.kanmamba=nn.ModuleList()
        # for i in range(2):
        #     self.kanmamba.append(kanmamba(self.dim_list[i],self.size[i]))
        self.memory_blocks = nn.ModuleList()
        ...
        self.mamba=ConvSSM(hidden_dim=512)
        self.CBAM=CBAM(512)
        self.vit=VisionTransformer(2,1,8,512,8,12)
    def forward(self, x, fadein_weights=[1.0, 1.0, 1.0, 1.0]):
        ...
        for i in range(self.level):
            ...
            # print(x.shape, i, "上第i层")
            # if i<2:
            #     x1=self.kanmamba[i](x)
            #     x1=self.memory_blocks1[i](x1)
            #     if isinstance(x1, tuple):  # memory block returns feature and attention weights
            #         x1, _ = x1
