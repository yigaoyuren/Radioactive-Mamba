...
class Zhang(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        if self.train:
            ...
            neg_items = os.listdir(os.path.join(self.root, 'pneumonia_256'))
            total = len(pos_items)
            num_pos = int(total * self.positive_ratio)
            num_neg = total - num_pos
            for item in pos_items[:num_pos]:
                image = Image.open(os.path.join(self.root, 'normal_256', item)).resize(self.img_size)
                self.data.append((image, 0))
                self.fnames.append(item)
            for item in neg_items[:num_neg]:
                image = Image.open(os.path.join(self.root, 'pneumonia_256', item)).resize(self.img_size)
